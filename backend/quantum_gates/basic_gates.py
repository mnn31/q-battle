...
def apply_damage_roll(base_damage):
    """Apply Pokémon-style damage roll: ±3% random variation"""
    if base_damage <= 0:
        return base_damage
    
    # Calculate ±3% range
    min_damage = max(1, int(base_damage * 0.97))
    max_damage = int(base_damage * 1.03)
    
    # Random roll within the range (inclusive)
